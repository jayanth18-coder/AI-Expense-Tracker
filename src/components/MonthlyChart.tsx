...
const MonthlyChart = ({ expenses = [], theme = "dark" }) => {
  ...
  const options = {
    ...
    scales: {
      x: {
        ticks: { color: theme === "dark" ? "#eee" : "#232323", font: { weight: "bold" } },
        grid: { display: false }
      },
      y: {
        beginAtZero: true,
        ticks: { color: theme === "dark" ? "#eee" : "#232323" },
        grid: { color: "#41407F22" }
      }
    },
    responsive: true,
  ...
  if (labels.length === 0) {
    return <div className="text-center text-zinc-400 py-8">No monthly expenses</div>;
  }
